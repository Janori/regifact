...
export class DashboardProvidersComponent implements OnInit {
  ...
  ngOnInit() {
    this.activatedRoute.queryParams.subscribe(data=>{
      ...
      if(data.query){
        ...
        this.searchService
        .search("users/p", `from=${this.params.from}&count=${this.params.to}&email=${this.params.query}&username=${this.params.query}&name=${this.params.query}`)
        .subscribe(data=>{
        ...
        this.usersService.getAllProviders(this.params.from, this.params.to).subscribe(data=>{
          this.users = data.data;
          this.resultCount = data.msg;
        });
      }
  ...
  buscar(value:string){
    this.router.navigate(['dashboard/providers'], {queryParams: {query: value}});
  }
